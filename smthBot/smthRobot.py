...
class SmthBot(GtalkRobot):
    ...
    #This method is used to send email for users.
    def command_003_Seach(self, user, message, args):
        '''[s|search|se]\s+(.*?@.+?)\s+(.*?),\s*(.*?)(?i)'''
        keyword = args[0]
        board = args[1]
        sort = args[2]
        ...
        self.replyMessage(user, "\nYour query string is "+ keyword +" at: "+time.strftime("%Y-%m-%d %a %H:%M:%S", time.gmtime()))
...
if __name__ == "__main__":
    bot = SmthBot()
    bot.setState('available', "Smth Gtalk Robot")
    bot.start("smth.robot@gmail.com", "smth2Power")
